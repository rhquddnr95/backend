...
1. 변화에 대응하는 OCP(Open-Closed Principle)
...
2. 데이터 불변성: Record 활용
신규 서비스일수록 데이터 정합성이 중요합니다. 주문(Order)이나 회원(Member)처럼 도메인 간에 전달되는 데이터가 도중에 수정되어 버그가 발생하는 것을 방지하기 위해 Java 17의 record를 적극 도입했습니다.
한 번 생성된 데이터는 결제가 완료될 때까지 상태가 변하지 않음을 보장하므로, 비즈니스 로직이 헤비해 지더라도 데이터의 신뢰도를 유지할 수 있습니다.

3. 이력 관리 전략: Snapshot & Audit
...
💡 실행 방법
프로젝트를 빌드합니다: ./gradlew build
애플리케이션을 실행합니다: BackendApplication 실행
H2 콘솔 확인: http://localhost:8080/h2-console (JDBC URL: jdbc:h2:mem:testdb)
